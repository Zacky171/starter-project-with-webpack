...
🔐 Authentication Flow
User visits /stories → Guard redirects to /login
Login → Token saved to localStorage
Protected pages now send Authorization: Bearer <token>
No 401 errors - all API calls guarded
📱 PWA Features
...
Unauth → stories = redirect ✓
Auth → stories = 200 OK ✓
Offline = cached assets ✓
SW no 401 interference ✓
📄 License
